...
var NgGrid = NgGrid_1 = (function () {
    ...
    function NgGrid(_differs, _ngEl, _renderer, componentFactoryResolver, _containerRef) {
        ...
        this._config = NgGrid_1.CONST_DEFAULT_CONFIG;
    }
    ...
    NgGrid.prototype.removeItem = function (ngItem) {
        ...
        for (var x_1 = 0; x_1 < this._items.length; x_1++) {
            if (this._items[x_1] == ngItem) {
                this._items.splice(x_1, 1);
            }
        }
        if (this._destroyed)
            return;
        ...
        while (items.length > 0) {
            ...
            while (items.length > 0) {
                ...
                if (item.row > currentRow)
                    break;
                var fits = false;
                for (var x_2 in tempColumns) {
                    if (item.sizex <= tempColumns[x_2]) {
                        tempColumns[x_2] -= item.sizex;
                        fits = true;
                        break;
                    }
                    else if (item.sizex > tempColumns[x_2]) {
                        tempColumns[x_2] = 0;
                    }
                }
                if (fits) {
            ...
            var newRow = 0;
            for (var x_3 in columnMax) {
                if (columnMax[x_3] > currentRow && (newRow == 0 || columnMax[x_3] < newRow)) {
                    newRow = columnMax[x_3];
                }
            }
    ...
    NgGrid.prototype._removeFromGrid = function (item) {
        for (var y_1 in this._itemGrid)
            for (var x_4 in this._itemGrid[y_1])
                if (this._itemGrid[y_1][x_4] == item)
                    delete this._itemGrid[y_1][x_4];
    };
